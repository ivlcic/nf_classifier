...
def prep_data(args, confs: List[Dict]) -> None:
    for conf in confs:
        type = conf.get('type')
        if type == 'wikiann':
            ner_conll_idx = nf.args.chech_param(conf, 'ner_conll_idx')
            target_base_name = nf.args.chech_param(conf, 'result_name')
            map_filter = nf.args.chech_param(conf, 'map_filter')
            lang = nf.args.chech_param(conf, 'lang')
            zip_fname = nf.args.chech_dir_param(conf, 'zip', args.corpora_dir)
            zip_dir = os.path.join(nf.default_tmp_dir, target_base_name)
            if not os.path.exists(zip_dir):
                os.mkdir(zip_dir)
            zipfile.ZipFile(zip_fname).extractall(zip_dir)
            proc_fname = nf.args.chech_dir_param(conf, 'proc_file', nf.default_tmp_dir)
